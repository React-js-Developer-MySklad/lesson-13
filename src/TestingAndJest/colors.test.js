import {ColorError, hex2rgb, toHex} from './colors';
...
describe('colors', () => {
  ...
  describe('hex2rgb', () => {
    ...
    it('complex', () => {
      expect(hex2rgb('#aBcDeA')).toMatchSnapshot();
    });

    it('check out validation', () => {
      expect(() => hex2rgb('#ffFFfff')).toThrow(ColorError);
    });

    it.skip('jest execution failed', () => {
      expect(() => hex2rgb('#ffF')).toThrow('invalid hex color: #ffFFff');
    });
  });
});
